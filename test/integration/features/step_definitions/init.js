...
var environment = config['hdfcapi-hdfcteam-demo'].env;
...
console.log('&$hdfcapi-hdfcteam-demo api: [' +  domains[environment] + ', ' + config['hdfcapi-hdfcteam-demo'].basepath + ']');

BeforeAll("BeforeFeatures", function(next) {
    if (apps[0].name) {
        console.log ("Getting Creds from DevAppsKeys.json");
        getCreds(config['hdfcapi-hdfcteam-demo'].app, config['hdfcapi-hdfcteam-demo'].product);
    } else
    {
...
Before(function(scenario, callback) {
    ...
    this.apickli = new apickli.Apickli(tokens[0],
        tokens[1]+ config['hdfcapi-hdfcteam-demo'].basepath,
        './test/integration/features/fixtures/');
    callback();
});
